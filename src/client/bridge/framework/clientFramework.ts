...
export class ClientFramework extends Framework {
    ...
    getPlayerInventory(): unknown {
        if (this.inventory) return this.inventory.getPlayerInventory();
    ...
    hasItem(item: string, amount: number): boolean {
        if (this.inventory) return this.inventory.hasItem(item, amount);
        ...
        for (const key in inventory) {
            ...
            if (itemData && itemData.name === item) {
                const count = itemData.count || itemData.amount || 0;
                return count >= amount;
            }
        }
        return false;
